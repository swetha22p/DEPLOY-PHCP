...
const handleSubmit = async () => {
    ...
    try {
        const formData = {
            ...
            _id : Date.now() + Math.random().toString(36).substring(2)// Simple unique ID
  ...
  try {
      ...
      const db = await openDB('formDataDB', 1, {
          upgrade(db) {
              ...
              db.createObjectStore('formDataStore',  { keyPath: '_id' },{ autoIncrement: true });
          },
      });
...
const fetchLatestDocuments = async () => {
  try {
    ...
    // Open IndexedDB database
    const db = await openDB('latest', 1, {
      upgrade(db) {
        // Create an object store if not exists
        const store = db.createObjectStore('latestDoc', { keyPath: '_id', autoIncrement: true });
      },
    });
    
    // Add timestamped data to object store
    const transaction = db.transaction('latestDoc', 'readwrite');
    const objectStore = transaction.objectStore('latestDoc');
    for (const document of latestDocuments) {
      // Simplify the _id field to a string
      document._id = document._id.$oid;
      try {
        await objectStore.add(document);
        console.log('Document added:', document._id);
      } catch (error) {
        console.error('Error adding document to IndexedDB:', error);
        console.log('Problematic document:', document);
      }
    }
    await transaction.done;
    
    console.log('Form data stored latest docs');
  } catch (error) {
    console.error('Error storing form data in IndexedDB:', error);
  }
};
